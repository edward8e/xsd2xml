...
class GenXML:
    def __init__(self, xsd, elem, enable_choice, schema_dir):
        self.xsd = xmlschema.XMLSchema(xsd, base_url=schema_dir)  # Use base_url for dependent schemas
        self.elem = elem
        self.enable_choice = enable_choice
        self.root = True
        self.vals = {}
        self.schema_dir = schema_dir  # Store the schema directory
...
def main():
    ...
    parser.add_argument("-d", "--schema-dir", dest="schema_dir", required=True,
                        help="select the directory containing dependent schemas")
    parser.add_argument("-c", "--choice",
                        action="store_true", dest="enable_choice", default=False,
                        help="enable the <choice> mode")
    args = parser.parse_args()

    generator = GenXML(args.xsdfile, args.element, args.enable_choice, args.schema_dir)
    generator.run()
